...
\chapter*{Übersicht zum Datenpaket "`${dataPackage.title.de}"'}
\markboth{\truncate{166mm}{${dataPackage.title.de}}}{}
...
\hline</#if>
<#if surveys?has_content>\textbf{Erhebungen} & <#list surveys as survey>\textbf{Erhebung ${survey?counter}:} ${survey.title.de}<#sep> \newline </#sep></#list> \vspace{0.18cm} \\
\hline</#if>
<#if populationDescriptionMapDe?has_content>\textbf{Grundgesamtheit} & <#list populationDescriptionMapDe?keys as population>\textbf{Erhebung <#list populationDescriptionMapDe?api.get(population) as surveyNumber>${surveyNumber}<#sep>, </#sep></#list>:} \newline ${population}<#sep> \newline </#sep></#list> \vspace{0.18cm} \\
\hline</#if>
<#if surveySampleMapDe?has_content>\textbf{Stichprobe} & <#list surveySampleMapDe?keys as sample>\textbf{Erhebung <#list surveySampleMapDe?api.get(sample) as surveyNumber>${surveyNumber}<#sep>, </#sep></#list>:} \newline ${sample}<#sep> \newline </#sep></#list> \vspace{0.18cm} \\
\hline</#if>
<#if surveyMethodMapDe?has_content>\textbf{Erhebungsmethode} & <#list surveyMethodMapDe?keys as surveyMethod>\textbf{Erhebung <#list surveyMethodMapDe?api.get(surveyMethod) as surveyNumber>${surveyNumber}<#sep>, </#sep></#list>:} ${surveyMethod}<#sep> \newline </#sep></#list> \vspace{0.18cm} \\
\hline</#if>
<#if surveyPeriodMap?has_content>\textbf{Feldzeit} & <#list surveyPeriodMap?keys as period>\textbf{Erhebung <#list surveyPeriodMap?api.get(period) as surveyNumber>${surveyNumber}<#sep>, </#sep></#list>:} \daterange{${period.start}}{${period.end}}<#sep> \newline </#sep></#list> \vspace{0.18cm} \\
\hline</#if>
<#if surveys?has_content>\textbf{Nettostichprobe} & <#list surveys as survey>\textbf{Erhebung ${survey?counter}:} n = \num{${survey.sampleSize}}<#sep> \newline </#sep></#list> \vspace{0.18cm} \\
\hline</#if>
<#if cleanedResponseRatesMap?has_content>\textbf{Rücklaufquote} & <#list cleanedResponseRatesMap?keys as surveyNumber>\textbf{Erhebung ${surveyNumber}:} \num[round-mode=places,round-precision=2]{${cleanedResponseRatesMap?api.get(surveyNumber)}} \%<#sep> \newline </#sep></#list> \vspace{0.18cm} \\
\hline</#if>
<#if surveyAnnotationsMapDe?has_content>\textbf{Anmerkungen zu den Erhebungen} & <#list surveyAnnotationsMapDe?keys as annotation>\textbf{Erhebung <#list surveyAnnotationsMapDe?api.get(annotation) as surveyNumber>${surveyNumber}<#sep>, </#sep></#list>:} \newline ${annotation}<#sep> \newline </#sep></#list> \vspace{0.18cm} \\
\hline</#if>
