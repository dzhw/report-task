...
\chapter*{Overview of Data Package "`${dataPackage.title.en}"'}
\markboth{\truncate{166mm}{${dataPackage.title.en}}}{}
...
\hline
\textbf{Sponsored by} & <#list dataPackage.sponsors as sponsor><#if sponsor.en??>${sponsor.en}<#else>${sponsor.de}</#if><#sep> \newline </#sep></#list> \vspace{0.18cm} \\
\hline
...
\hline</#if>
<#if surveys?has_content>\textbf{Surveys} & <#list surveys as survey>\textbf{Survey ${survey?counter}:} ${survey.title.en}<#sep> \newline </#sep></#list> \vspace{0.18cm} \\
\hline</#if>
<#if populationDescriptionMapEn?has_content>\textbf{Population} & <#list populationDescriptionMapEn?keys as population>\textbf{Survey <#list populationDescriptionMapEn?api.get(population) as surveyNumber>${surveyNumber}<#sep>, </#sep></#list>:} \newline ${population}<#sep> \newline </#sep></#list> \vspace{0.18cm} \\
\hline</#if>
<#if surveySampleMapEn?has_content>\textbf{Sampling Procedure} & <#list surveySampleMapEn?keys as sample>\textbf{Survey <#list surveySampleMapEn?api.get(sample) as surveyNumber>${surveyNumber}<#sep>, </#sep></#list>:} \newline ${sample}<#sep> \newline </#sep></#list> \vspace{0.18cm} \\
\hline</#if>
<#if surveyMethodMapEn?has_content>\textbf{Survey Method} & <#list surveyMethodMapEn?keys as surveyMethod>\textbf{Survey <#list surveyMethodMapEn?api.get(surveyMethod) as surveyNumber>${surveyNumber}<#sep>, </#sep></#list>:} ${surveyMethod}<#sep> \newline </#sep></#list> \vspace{0.18cm} \\
\hline</#if>
<#if surveyPeriodMap?has_content>\textbf{Field Period} & <#list surveyPeriodMap?keys as period>\textbf{Survey <#list surveyPeriodMap?api.get(period) as surveyNumber>${surveyNumber}<#sep>, </#sep></#list>:} \daterange{${period.start}}{${period.end}}<#sep> \newline </#sep></#list> \vspace{0.18cm} \\
\hline</#if>
<#if surveys?has_content>\textbf{Net Sample Size} & <#list surveys as survey>\textbf{Survey ${survey?counter}:} n = \num{${survey.sampleSize}}<#sep> \newline </#sep></#list> \vspace{0.18cm} \\
\hline</#if>
<#if cleanedResponseRatesMap?has_content>\textbf{Response Rate} & <#list cleanedResponseRatesMap?keys as surveyNumber>\textbf{Survey ${surveyNumber}:} \num[round-mode=places,round-precision=2]{${cleanedResponseRatesMap?api.get(surveyNumber)}} \%<#sep> \newline </#sep></#list> \vspace{0.18cm} \\
\hline</#if>
<#if surveyAnnotationsMapEn?has_content>\textbf{Remarks on the Surveys} & <#list surveyAnnotationsMapEn?keys as annotation>\textbf{Survey <#list surveyAnnotationsMapEn?api.get(annotation) as surveyNumber>${surveyNumber}<#sep>, </#sep></#list>:} \newline ${annotation}<#sep> \newline </#sep></#list> \vspace{0.18cm} \\
\hline</#if>
