	...
	<#if variable.distribution?? && (variable.distribution.validResponses?? || variable.distribution.missings??)>
		<#if variable.scaleLevel??>
				...
				<#if variable.dataType.en == "string">
					...
					\multicolumn{1}{X}{<#if !anonymizeValidResponses[variable.id] || validResponse.absoluteFrequency gte 10>${validResponse.value!"-"}<#else>anonymized</#if>} &
					<#if !anonymizeValidResponses[variable.id] || validResponse.absoluteFrequency gte 10>${(validResponse.label.en)!"-"}<#else>anonymized</#if> &
					...
					<#else>-\\
						...
						<#else>
							<#list firstTenValidResponses[variable.id] as validResponse>
								\multicolumn{1}{X}{<#if !anonymizeValidResponses[variable.id] || validResponse.absoluteFrequency gte 10>${validResponse.value!"-"}<#else>anonymized</#if>} & <#if !anonymizeValidResponses[variable.id] || validResponse.absoluteFrequency gte 10>${(validResponse.label.en)!"-"}<#else>anonymized</#if> & \num{${validResponse.absoluteFrequency!"-"}} & \num[round-mode=places,round-precision=2]{${validResponse.validRelativeFrequency!"-"}} & \num[round-mode=places,round-precision=2]{${validResponse.relativeFrequency!"-"}} \\
							</#list>
							... & ... & ... & ... & ... \\
				...
				<#else>
					...
					<#if !anonymizeValidResponses[variable.id] || validResponse.absoluteFrequency gte 10>${validResponse.value!"-"}<#else>anonymized</#if> &
				% TODO try size/length gt 0; take over for other passages
					\multicolumn{1}{X}{<#if !anonymizeValidResponses[variable.id] || validResponse.absoluteFrequency gte 10>${(validResponse.label.en)!"-"}<#else>anonymized</#if>} &
					...
					<#else>
						...
						<#else>
							<#list firstTenValidResponses[variable.id] as validResponse>
								<#if !anonymizeValidResponses[variable.id] || validResponse.absoluteFrequency gte 10>${validResponse.value!"-"}<#else>anonymized</#if> & \multicolumn{1}{X}{<#if !anonymizeValidResponses[variable.id] || validResponse.absoluteFrequency gte 10>${(validResponse.label.en)!"-"}<#else>anonymized</#if>} & %${validResponse.absoluteFrequency!"-"} &
								<#if validResponse.absoluteFrequency??>
								  \num{${validResponse.absoluteFrequency}} &
								<#else>
								  - &
