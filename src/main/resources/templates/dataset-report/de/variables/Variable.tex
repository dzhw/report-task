	...
	<#if variable.distribution?? && (variable.distribution.validResponses?? || variable.distribution.missings??)>
		<#if variable.scaleLevel??>
				...
				<#if variable.dataType.de == "string">
					...
					\multicolumn{1}{X}{<#if !anonymizeValidResponses[variable.id] || validResponse.absoluteFrequency gte 10>${validResponse.value!"-"}<#else>anonymisiert</#if>} &
					<#if !anonymizeValidResponses[variable.id] || validResponse.absoluteFrequency gte 10>${(validResponse.label.de)!"-"}<#else>anonymisiert</#if>
					...
					<#else>-\\
						...
						<#else>
							<#list firstTenValidResponses[variable.id] as validResponse>
								\multicolumn{1}{X}{<#if !anonymizeValidResponses[variable.id] || validResponse.absoluteFrequency gte 10>${validResponse.value!"-"}<#else>anonymisiert</#if>} & <#if !anonymizeValidResponses[variable.id] || validResponse.absoluteFrequency gte 10>${(validResponse.label.de)!"-"}<#else>anonymisiert</#if> & \num{${validResponse.absoluteFrequency!"-"}} & \num[round-mode=places,round-precision=2]{${validResponse.validRelativeFrequency!"-"}} & \num[round-mode=places,round-precision=2]{${validResponse.relativeFrequency!"-"}} \\
							</#list>
							... & ... & ... & ... & ... \\
				...
				<#else>
					...
					<#if !anonymizeValidResponses[variable.id] || validResponse.absoluteFrequency gte 10>${validResponse.value!"-"}<#else>anonymisiert</#if> &
				% TODO try size/length gt 0; take over for other passages
					\multicolumn{1}{X}{<#if !anonymizeValidResponses[variable.id] || validResponse.absoluteFrequency gte 10>${(validResponse.label.de)!"-"}<#else>anonymisiert</#if>} &
					...
					<#else>
						...
						<#else>
							<#list firstTenValidResponses[variable.id] as validResponse>
								<#if !anonymizeValidResponses[variable.id] || validResponse.absoluteFrequency gte 10>${validResponse.value!"-"}<#else>anonymisiert</#if> & \multicolumn{1}{X}{<#if !anonymizeValidResponses[variable.id] || validResponse.absoluteFrequency gte 10>${(validResponse.label.de)!"-"}<#else>anonymisiert</#if>} & %${validResponse.absoluteFrequency!"-"} &
								<#if validResponse.absoluteFrequency??>
								  \num{${validResponse.absoluteFrequency}} &
								<#else>
								  - &
                ...
                <#if variable.distribution.validResponses??>
                    <#assign measures = []/>
                    <#if variable.distribution.statistics.minimum?? && !anonymizeValidResponses[variable.id]>
                      <#assign measures = measures + [{"name":"Minimum ($min$)",
                      	 "value":"${variable.distribution.statistics.minimum}"}] />
                    </#if>
                   	<#if variable.distribution.statistics.maximum?? && !anonymizeValidResponses[variable.id]>
